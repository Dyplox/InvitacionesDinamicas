  ...
  return (
    <>
      ...
      <div className="flex min-h-[calc(100vh-73px)] overflow-hidden">
        ...
        <aside className="w-[380px] bg-surface-container-lowest border-l border-outline-variant/15 flex flex-col p-8 overflow-y-auto hidden lg:flex h-full gap-10">
            {activeObject ? (
               <div className="space-y-8 flex-1">
                  <div className="flex items-center justify-between border-b border-outline-variant/20 pb-4">
                     <h2 className="font-headline text-2xl text-on-surface">Content & Variables</h2>
                     <span className="bg-tertiary-container/30 text-on-tertiary-container px-3 py-1 rounded-full font-label text-[10px] uppercase tracking-tighter font-semibold">Live Mode</span>
                  </div>
                  ...
                  {(activeObject.type === 'i-text' || activeObject.type === 'textbox') && (
                     <section className="space-y-6">
                        <div className="flex items-center gap-2 text-primary font-label text-[10px] uppercase tracking-widest font-bold">
                           <span className="material-symbols-outlined text-sm">match_case</span>
                           Typography (Content)
                        </div>
                        <div className="space-y-4">
                           <div>
                              <label className="block font-label text-xs text-on-surface-variant mb-2">Font Family</label>
                              <select
                                 className="w-full bg-white border border-outline-variant/30 rounded-lg px-4 py-3 font-headline text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all text-on-surface"
                           ...
                           <div className="grid grid-cols-2 gap-4">
                              <div>
                                 <label className="block font-label text-xs text-on-surface-variant mb-2">Font Size</label>
                                 <div className="relative">
                                    <input
                                       type="number"
                                       value={fontSize}
                                       onChange={(e) => handleUpdateText('fontSize', parseInt(e.target.value) || 32)}
                                       className="w-full bg-white border border-outline-variant/30 rounded-lg px-4 py-3 font-body text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none text-on-surface"
                                    />
                                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-outline font-label uppercase">PX</span>
                                 </div>
                              </div>
                              <div>
                                 <label className="block font-label text-xs text-on-surface-variant mb-2">Line Height</label>
                                 <div className="relative">
                                    <input
                                       type="number"
                                       step="0.1"
                                       value={(activeObject as fabric.IText).lineHeight || 1.16}
                                       onChange={(e) => handleUpdateText('lineHeight', parseFloat(e.target.value) || 1.16)}
                                       className="w-full bg-white border border-outline-variant/30 rounded-lg px-4 py-3 font-body text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none text-on-surface"
                                    />
                                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-outline font-label uppercase">EM</span>
                                 </div>
                              </div>
                           </div>
                           <div>
                              <label className="block font-label text-xs text-on-surface-variant mb-2">Color Palette</label>
                              <div className="flex gap-3">
                                 <button onClick={() => handleUpdateText('fill', '#1b1c1d')} className={`w-10 h-10 rounded-full bg-[#1b1c1d] transition-transform hover:scale-110 ${textColor === '#1b1c1d' ? 'ring-4 ring-primary/20 border-2 border-primary' : ''}`}></button>
                                 <button onClick={() => handleUpdateText('fill', '#094cb2')} className={`w-10 h-10 rounded-full bg-[#094cb2] transition-transform hover:scale-110 ${textColor === '#094cb2' ? 'ring-4 ring-primary/20 border-2 border-primary' : ''}`}></button>
                                 <button onClick={() => handleUpdateText('fill', '#6d5e00')} className={`w-10 h-10 rounded-full bg-[#6d5e00] transition-transform hover:scale-110 ${textColor === '#6d5e00' ? 'ring-4 ring-primary/20 border-2 border-primary' : ''}`}></button>
                                 <button onClick={() => handleUpdateText('fill', '#e3e2e3')} className={`w-10 h-10 rounded-full bg-[#e3e2e3] border border-outline-variant/20 transition-transform hover:scale-110 ${textColor === '#e3e2e3' ? 'ring-4 ring-primary/20 border-2 border-primary' : ''}`}></button>
                                 <button onClick={() => handleUpdateText('fill', '#ba1a1a')} className={`w-10 h-10 rounded-full bg-[#ba1a1a] transition-transform hover:scale-110 ${textColor === '#ba1a1a' ? 'ring-4 ring-primary/20 border-2 border-primary' : ''}`}></button>
                              </div>
                           </div>
                        </div>
                     </section>
                     ...
                     <section className="space-y-6 pt-6 border-t border-outline-variant/10">
                        ...
                        <div className="space-y-4">
                           <div className="space-y-1.5">
                              <label className="block font-label text-xs text-on-surface-variant">Link URL</label>
                              <div className="flex items-center bg-white border border-outline-variant/30 rounded-lg px-3 focus-within:border-primary transition-all text-on-surface">
                                 <span className="material-symbols-outlined text-outline text-lg">link</span>
                                 <input
                                    type="text"
                                    className="w-full bg-transparent border-none focus:ring-0 py-3 text-sm font-body truncate outline-none px-2"
                                    placeholder="https://..."
                                    value={linkUrl}
            ...
            {/* Action Button */}
            <button className="mt-4 w-full py-4 bg-surface-container-high hover:bg-surface-container-highest transition-colors flex items-center justify-center gap-3 rounded-xl group text-on-surface font-bold text-xs uppercase tracking-widest">
               Review Changes
